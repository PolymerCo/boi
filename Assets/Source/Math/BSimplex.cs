...
namespace Source.Math {
    public static class BSimplex
    {
        ...
        /** Standard offset between simplex iterations. Ensures that values close to 0 do not have repeated features. */
        private static readonly (int, int) IterationOffset = (6745, 4003);
     ...
     */
        ...
        /// <summary>
        /// Gets iterated brownian noise a given position.
        /// </summary>
        /// <param name="seed">The seed for the noise.</param>
        /// <param name="x">The X position.</param>
        /// <param name="y">The Y position.</param>
        /// <param name="frequency">The frequency of the base noise pass.</param>
        /// <param name="iterations">The iterations of brownian noise to add.</param>
        /// <param name="persistence">The amplitude persistence between iterations.</param>
        /// <param name="frequencyGain">The gain of the frequency between iterations.</param>
        /// <returns>A noise value between 0 and 1.</returns>
        public static float Brownian01(long seed, double x, double y, double frequency, int iterations, float persistence, double frequencyGain) {
            float maxAmplitude = 0;
            float amplitude = 1;
            float noise = 0;
            
            for (int i = 0; i < iterations; i++) {
                noise += Noise2(seed, x * frequency, y * frequency) * amplitude;
                maxAmplitude += amplitude;
                amplitude *= persistence;
                frequency *= frequencyGain;
                x += IterationOffset.Item1;
                y += IterationOffset.Item2;
            }

            noise /= maxAmplitude;

            return noise / 2f + 0.5f;
        }
